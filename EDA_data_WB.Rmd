...
- I work on a github repository ([/DiegoKoz/datathonuser2019](https://github.com/DiegoKoz/datathonuser2019)) so anyone can acces the code and re-use it.
...
Here, I also added the option of the two hierarchical variables from the dataset (region and income group), and add the posibility of doing a lineal smoothing (lineal regression) by group, in order to modelate the relation between the variables. 
...
Besides the plots, I consider it's usefull for the users to have the raw data at hand, for a quick inspection. For this, I used the library [DT](https://github.com/rstudio/DT) which include funcionalities usefull for the user to explore the data. Each of the tabs have the corresponding table. This is especially usefull for the treemaps, which shows a three-coded label for countries, and with the raw data, users can check which country is which. 
